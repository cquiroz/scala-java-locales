...
object CodeGenerator {
  ...
  def buildClassTree(root: XMLLDML, langs: List[List[String]])
    ...
    LAZYVAL(ldml.scalaSafeName, "LDML") :=
      Apply(ldmlSym, parent, ldmlLocaleTree, defaultNS, LIST(numericSymbols))
  }
...
object ScalaLocaleCodeGen {
  ...
  def generateLocaleData(base: File, data: File): Seq[File] = {
    ...
    val f3 = writeGeneratedTree(base, "minimal", stdTree)
    val f4 = writeGeneratedTree(base, "all", tree)
