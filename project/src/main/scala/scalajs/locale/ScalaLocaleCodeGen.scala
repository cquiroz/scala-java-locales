...
object CodeGenerator {
  ...
  def numericSystems(ns: Seq[NumericSystem]): Tree = {
    ...
    BLOCK (
      IMPORT("scala.scalajs.locale.ldml.NumberingSystem"),
...
object ScalaLocaleCodeGen {
  ...
  def constructLDMLDescriptor(f: File, xml: Elem, ns: Map[String, NumericSystem]): XMLLDML = {
    ...
    val symbols = (xml \ "numbers" \\ "symbols").flatMap { s =>
      ...
      val symbols = s.collect {
        case s @ <symbols>{_*}</symbols> if (s \ "alias").isEmpty =>
          ...
          val sym = NumberSymbols(sns, decimal, group, list,
