  ...
  def numericSystems(ns: Seq[LDMLNumericSystem]): Tree = {
    BLOCK (
      OBJECTDEF("numericsystems") := BLOCK(
        LAZYVAL("numericsystems", "Array[(String, String)]") := ARRAY(ns.map(i => TUPLE(LIT(i.id), LIT(i.digits)))) withComment autoGeneratedCommend
      )
    ) inPackage "scala.scalajs.locale.ldml.data"
  }
...
object ScalaLocaleCodeGen {
  ...
  def generateLocaleData(base: File, data: File): Seq[File] = {
    ...
    val numberingSystems = data.toPath.resolve("common").resolve("supplemental").resolve("numberingSystems.xml").toFile
    val numericSystems = parseNumberingSystems(XML.withSAXParser(parser).loadFile(numberingSystems))
    ...
    val f4 = writeGeneratedTree(base, "numericsystems", CodeGenerator.numericSystems(numericSystems))
