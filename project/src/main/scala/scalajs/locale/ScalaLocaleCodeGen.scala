...
import java.util.function.{IntPredicate, IntSupplier}
...
object CodeGenerator {
  ...
  def buildClassTree(packageObject: String, ldmls: List[XMLLDML], only: List[String]): Tree = {
    ...
    BLOCK (
      ...
      IMPORT("scala.scalajs.locale.ldml.Symbols"),
  ...
  def buildClassTree(root: XMLLDML, langs: List[List[String]])(ldml: XMLLDML): Tree = {
    ...
    val numericSymbols = ldml.digitSymbols.map { case (ns, symb) =>
      ...
      Apply(ldmlNumericSym, REF(ns.id), symb.aliasOf.fold(NONE)(n => SOME(REF(n.id))),
        decimal, group, list, percent, minus, perMille, infinity, nan, exp)
    }

    LAZYVAL(ldml.scalaSafeName, "LDML") := Apply(ldmlSym, parent, ldmlLocaleTree, defaultNS, LIST(numericSymbols))
  }
  ...
  def numericSystems(ns: Seq[NumericSystem]): Tree = {
    ...
    BLOCK (
      IMPORT("scala.scalajs.locale.ldml.NumberingSystem") withComment autoGeneratedCommend,
...
object ScalaLocaleCodeGen {
  ...
  def constructLDMLDescriptor(f: File, xml: Elem, latn: NumericSystem, ns: Map[String, NumericSystem]): XMLLDML = {
  ...
  def generateLocaleData(base: File, data: File): Seq[File] = {
    ...
    val clazzes = for {
      ...
      //if f.getName == "ar.xml" || f.getName == "root.xml"
