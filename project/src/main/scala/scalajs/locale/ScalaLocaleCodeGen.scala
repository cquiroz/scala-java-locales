...
object CodeGenerator {
  ...
  def treeHugLdml(ldmls: List[XMLLDML]): Tree = {
    ...
    BLOCK (
      ...
      PACKAGEOBJECTDEF("locales") := BLOCK(
        ldmls.map(treeHugLdml(root, langs))
      )
  ...
  def treeHugLdml(root: XMLLDML, langs: List[List[String]])(ldml: XMLLDML): Tree = {
...
object ScalaLocaleCodeGen {
  ...
  def generateLocaleData(base: File, data: File): Seq[File] = {
    ...
    val tree = CodeGenerator.treeHugLdml(clazzes)
