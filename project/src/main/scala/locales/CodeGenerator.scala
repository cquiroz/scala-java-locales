...
object CodeGenerator {
  ...
  def currencyData(c: CurrencyData): Tree = {
    //val ldmlCurrencySym = getModule("CurrencySymbol")
    val ldmlCurrencyTypeSym = getModule("CurrencyType")
    val ldmlCurrencyFractionsInfoSym = getModule("CurrencyDataFractionsInfo")
    val ldmlCurrencyDataRegionSym = getModule("CurrencyDataRegion")
    val ldmlCurrencyDataRegionCurrencySym = getModule("CurrencyDataRegionCurrency")
    val ldmlCurrencyNumericCodeSym = getModule("CurrencyNumericCode")

    val currencyTypes: Seq[Tree] =
      (LAZYVAL("currencyTypes", "List[CurrencyType]") := LIST(c.currencyTypes.map(k => REF(k.scalaSafeName)))) +:
      c.currencyTypes.map { code: CurrencyType =>
        LAZYVAL(code.scalaSafeName, "CurrencyType") :=
          Apply(ldmlCurrencyTypeSym,
            LIT(code.currencyCode),
            LIT(code.currencyName)
          )
      }

    val currencyFractions: Seq[Tree] = (LAZYVAL("fractions", "List[CurrencyDataFractionsInfo]") := LIST(c.fractions.map(k => REF(k.scalaSafeName)))) +:
      c.fractions.map { info: CurrencyDataFractionsInfo =>
        LAZYVAL(info.scalaSafeName, "CurrencyDataFractionsInfo") :=
          Apply(ldmlCurrencyFractionsInfoSym,
    ...
    val currencyDataRegion: Seq[Tree] = (LAZYVAL("regions", "List[CurrencyDataRegion]") := LIST(c.regions.map(k => REF(k.scalaSafeName)))) +:
      c.regions.map { region: CurrencyDataRegion =>
        LAZYVAL(region.scalaSafeName, "CurrencyDataRegion") :=
          Apply(ldmlCurrencyDataRegionSym,
            LIT(region.countryCode),
            LIST(region.currencies.map{ currency: CurrencyDataRegionCurrency =>
              Apply(ldmlCurrencyDataRegionCurrencySym,
    ...
    val numericCodes: Seq[Tree] = (LAZYVAL("numericCodes", "List[CurrencyNumericCode]") := LIST(c.numericCodes.map(k => REF(k.scalaSafeName)))) +:
      c.numericCodes.map { code: CurrencyNumericCode =>
        LAZYVAL(code.scalaSafeName, "CurrencyNumericCode") :=
          Apply(ldmlCurrencyNumericCodeSym,
            LIT(code.currencyCode),
    ...
    BLOCK (
      ...
      OBJECTDEF("currencydata") := BLOCK(
        currencyTypes ++ currencyFractions ++ currencyDataRegion ++ numericCodes
      )
    ) inPackage "locales.cldr.data"
  }
}